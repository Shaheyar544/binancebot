...
class StrategyEngine:
    ...
    def evaluate(
        ...
        # Safety gate 3: Regime-aware blocks (BEAR, STRONG_BEAR, EVENT_RISK)
        if mtf.regime in {MarketRegime.BEAR, MarketRegime.STRONG_BEAR}:
            return DecisionSnapshot(
                decision_id=str(uuid.uuid4()),
                symbol=mtf.symbol,
                decision_state=DecisionState.BLOCKED,
        ...
        if mtf.regime == MarketRegime.NEUTRAL:
            effective_threshold = max(self.min_entry_score, Decimal("90.0"))
        elif mtf.regime == MarketRegime.HIGH_VOLATILITY:
            effective_threshold = max(self.min_entry_score, Decimal("90.0"))
...
class ExitManager:
    ...
    def evaluate_position(
        ...
        if not partial_tp_already_taken and risk_distance > Decimal("0"):
